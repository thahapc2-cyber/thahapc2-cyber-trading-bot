...
class PortfolioRiskManager:
    """Manage portfolio-level risk constraints and monitoring.
    ...
    def __init__(self, config: Dict):
        ...
        Args:
            config: Dict with keys:
                ...
                - max_correlated_risk: Max % risk in correlated pairs
                  (default 0.04)
                - max_positions: Max open positions (default 5)
                - max_drawdown: Max portfolio DD before trading halts
                  (default 0.10)
    ...
    async def validate_new_signal(
        ...
        """Validate if a new signal can be traded given portfolio risk.

        Args:
            signal: {pair, direction, entry, stop_loss, etc}
        ...
        current_risk = sum(
            p.risk_per_position * p.size for p in self.positions
        )
        if current_risk + signal_risk_pct > self.config['max_total_risk']:
            return False, (
                f"Total risk would exceed "
                f"{self.config['max_total_risk']*100}% "
                f"(current: {current_risk*100:.2f}%, "
                f"new: {signal_risk_pct*100:.2f}%)"
            )

        # 6. Correlated risk check
        correlated_risk = self._calculate_correlated_risk(signal['pair'])
        if (correlated_risk + signal_risk_pct >
                self.config['max_correlated_risk']):
            return False, (
                f"Correlated risk limit exceeded "
                f"(current: {correlated_risk*100:.2f}%, "
                f"new: {signal_risk_pct*100:.2f}%)"
            )
    ...
    def _calculate_safe_position_size(
        ...
        if entry <= 0 or stop_loss <= 0 or entry == stop_loss:
            logger.error("Invalid entry/SL: %s/%s", entry, stop_loss)
            return 0
        ...
        max_size = (
            account_balance * self.config['max_position_size'] / entry
        )
        ...
        logger.debug("Calculated position size: %.4f lots", position_size)
    ...
    def close_position(
        ...
        Args:
            pair: Trading pair
            exit_price: Price at exit (unused but kept for API compatibility)
        ...
        if position:
            ...
            logger.info(
                "Position closed: %s | P&L: %.4f | Daily P&L: %.4f",
                pair,
                pnl,
                sum(self.daily_pnl_list)
            )
    ...
    def update_position_pnl(
        ...
        Args:
            pair: Trading pair
            current_price: Current market price (unused but kept for
                API compatibility)
    ...
    def get_risk_metrics(self) -> RiskMetrics:
        ...
        win_rate = (
            wins / len(self.daily_pnl_list)
            if self.daily_pnl_list
            else 0
        )
        ...
        metrics = RiskMetrics(
            ...
            portfolio_leverage=(
                sum(p.entry_price * p.size for p in self.positions) /
                self.current_equity
            ),
            ...
            margin_utilization=(
                total_risk / self.config['max_total_risk']
            ),
    ...
    async def emergency_close_all(self, reason: str) -> bool:
        ...
        for position in positions_to_close:
            try:
                ...
                logger.info("Closing %s...", position.pair)
                success_count += 1
            except Exception as e:  # pylint: disable=broad-exception-caught
                logger.error("Failed to close %s: %s", position.pair, e)

        if self.on_drawdown_alert:
            await self.on_drawdown_alert(
                "EMERGENCY_CLOSE",
                f"Closed {success_count}/{len(positions_to_close)} "
